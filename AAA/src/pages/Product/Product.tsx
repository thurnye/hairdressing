...
const Product: FC<ProductProps> = () => {
  ...
  useEffect(() => {
    ...
    const paginatedData:any = data.slice(offset).slice(0, itemPerPage)
    setPerPageData(paginatedData)
  },[currentPage])
      ...
      <Box>
        <Box sx={{ flexGrow: 1, mt: 2, display: { xs: '', sm: 'block' }}}>
          <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 12, sm: 8, md: 12 }}>
            {perPageData.map((el:any, index:number) => (
